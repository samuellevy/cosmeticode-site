...
import { Actions, Button, Container, Content, DotButton, Dots, Item } from "./carousel.ui.style";
  ...
  return (
    <Container>
      <Content $spacing={$spacing} ref={ref}>
        <div>
          {slides.map((Slide, index) => (
            <Item $perView={$perView} $spacing={$spacing} key={index}>
              <Slide />
            </Item>
          ))}
        </div>
      ...
      <Actions $actionsMarginTop={$actionsMarginTop}>
        <Button disabled={prevBtnDisabled} onClick={onPrevButtonClick} type="button">
        ...
        <Button disabled={nextBtnDisabled} onClick={onNextButtonClick} type="button">
